...
async function getFacets() {
  const res = await fetch('https://www.vinmonopolet.no/vmpws/v2/vmp/search?q=:relevance:mainCategory:&searchType=product&fields=FULL')
  const searchResult = await res.json()
  return searchResult.productSearchResult.facets.map(f => new Facet(f))
}
...
module.exports = async (opts) => {
  ...
  const productHash = {}
  for (const {code, name, price, availability, mainSubCategory, mainCountry} of products) {
    let stockCount = 0
