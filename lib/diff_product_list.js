...
const { autolinkItemsInDiff } = require('./add_vintappd_metadata')
...
exports.runDiff = async (app, opts) => {
  ...
  Object.keys(productHash).forEach(code => {
    if (savedProductHash[code] && savedProductHash[code].time) {
      productHash[code].time = savedProductHash[code].time
    } else {
      productHash[code].time = Date.now()
    }
  })
  ...
  if (opts.vintappd) {
    await autolinkItemsInDiff(productDiff)
  }
